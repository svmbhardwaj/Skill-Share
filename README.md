<p align="center">
  <img src="assets/logo.svg" alt="SkillShare" width="300" />
</p>

# 🚀 SkillShare

> **Your local services marketplace** — find trusted help nearby, or offer what you're good at. 🤝

SkillShare is a **local services marketplace** where people find trusted help nearby and others offer what they're good at. 🏘️
...
| | | |
|---|---|---|
| ✨ [Key Features](#-key-features) | 🧭 [User Flows](#-user-flows) | 🛠️ [Technology Stack](#-technology-stack) |
| 🏗️ [Architecture](#-architecture) | 📁 [Repository Structure](#-repository-structure) | 🗄️ [Data Models](#-data-models) |
| 🔄 [Job Lifecycle](#-job-lifecycle) | 💳 [Payments](#-payments-razorpay-test-mode) | 🔐 [Authentication](#-authentication) |
| 📍 [Location](#-location) | 📡 [API Reference](#-api-reference) | 🔑 [Environment Variables](#-environment-variables) |
| 🛠️ [Local Development](#-local-development) | ⌨️ [Commands](#-commands) | 🔒 [Security Notes](#-security-notes) |
| 🧪 [Test Mode Notes](#-test-mode-payment-notes) | ⚠️ [Known Limitations](#-known-limitations) | 🔮 [Future Improvements](#-future-improvements) |
...
│   └── package.json
│
├── frontend/                     # ⚛️ Next.js app (Pages Router)
...
- 🎟️ **Tokens** — login/register/Google return an **access token** (1 h) and a **refresh token** (30 d). Refresh tokens carry a `type: 'refresh'` claim and can only be exchanged via `POST /api/auth/refresh-token` for a new access token.
...
- 🛡️ **Authorization** — protected endpoints (`Authorization: Bearer <token>`) verify the user, and controllers enforce ownership/role rules (e.g. only the job's client can pay, only its provider can accept).
- 🚪 **Logout** — the frontend discards tokens locally; `logout-everywhere` also invalidates them server-side.

> ⚠️ Tokens are stored in `localStorage` by the frontend for simplicity — this means any XSS in the browser could read them; keep this in mind and treat the client as untrusted (the backend never relies on client assertions for authorization).
...
> ⚠️ Never commit real `.env` files. Create them from the tables below with placeholder values.
...
> ℹ️ There are no automated tests in the repository at this time.
...
- 🛡️ `RAZORPAY_KEY_SECRET` and `JWT_SECRET` are used **only server-side**; the frontend only ever receives the public Razorpay key id.
- ⚖️ The backend is the source of truth for authorization and money amounts: it derives payment amounts from the stored Job and re-verifies Razorpay signatures instead of trusting the client.
...
- 🙅‍♂️ Never publish or commit real key values; only the public key id belongs in the frontend.
...
<p align="center">Made with ❤️ for local communities · SkillShare</p>